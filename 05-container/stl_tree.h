 ...
 * 根据红黑树规则4, X必为红, 若P亦为宏(这就违反了规则3,必须调整树形)
 * 则G必为黑(因为原树要符合规则3)
...
protected:
    ...
    Compare key_compare;    //节点间的简直大小比较准则,应该会是function object
    ...
    static reference value(link_type x) { return (link_type&)(x->left); }
    ...
    static reference value(base_ptr x) { return (link_type&)(x->left); }
...
rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__insert(base_ptr x_, base_ptr y_, const Value &v)
{
    ...
    //key_compare简直大小比较准则(function object)
